...
# Pressures and temperatures from 3.5.6 for Tambora period

# Compares several different ensemble members
...
Imagedir<-sprintf("%s/images/Tambora_by_member",Sys.getenv('SCRATCH'))
...
plot.hour<-function(n.count) {    
      ...
      n.date<-c.date+n.count/(24*1) # 60-minute timesteps
      ...
      prmsl.normal<-TWCR.get.slice.at.hour('prmsl',year,month,day,hour,version='3.4.1',
                                               type='normal')
         obs<-TWCR.get.obs(year,month,day,hour,version='3.5.6')
