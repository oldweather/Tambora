# Get reanalysis normals and sds for the Tambora obs locations
...
get.reanalysis<-function(dte) {
  ...
  return(list(v354=v354,indices=li$indices))
}
...
c.date<-chron(dates="1816/12/31",times="00:00:00",
...
#Reanalysis.normals<-list()
load('Reanalysis.normals.Rdata')
for(i in seq(1:length(Results))) {
   if(is.null(Results[[i]])) next
   Reanalysis.normals<-merge.reanalysis(Reanalysis.normals,Results[[i]])
}
save(Reanalysis.normals,file='Reanalysis.normals.Rdata')
