...
Draw.colourbar<-function(Trange=10) {

   pushViewport(plotViewport(margins=c(1,1,1,4)))
   pushViewport(dataViewport(c(0,1),
		             c(Trange/2*-1,Trange/2),
				      extension=0))
   grid.yaxis(main=FALSE)
   grid.text('Temperature anomaly (C)',x=unit(1.5,'npc'),rot=270)

   gp<-gpar(col=Options$land.colour,fill=Options$land.colour)
   grid.polygon(x=unit(c(0.5,0.8,0.8,0.5),'npc'),
                y=unit(c(Trange/2*-1,Trange/2*-1,Trange/2,Trange/2),'native'),
                gp=gp)

   for(i in seq(1,100)) {

      t2m<-Trange/2*2*(i-50.5)/100
      col=rgb(0,0,0,1)
      if(t2m>0) fill=rgb(1,0,0,min(0.99,t2m/Trange))
      else fill=rgb(0,0,1,min(0.99,-1*t2m/Trange))
      gp<-gpar(col=fill,fill=fill,lwd=1)
      hgt<-1/100*Trange
      grid.polygon(x=unit(c(0.5,0.8,0.8,0.5),'npc'),
                y=unit(c(t2m-hgt,t2m-hgt,t2m+hgt,t2m+hgt),'native'),
                gp=gp)
...
Draw.pressure<-function(mslp,Options,colour=c(0,0,0)) {
  ...
  if(!is.na(lines) && length(lines)>0) {
    ...
    image.name<-sprintf("panel+bar_%04d-%02d-%02d:%02d.png",Year,Month,Day,Hour)
   ...
   # Geometries
    ...
    pushViewport(viewport(width=0.90,height=1.0,x=0,y=0,
    ...
    pushViewport(viewport(width=0.15,height=1.0,x=0.85,y=0,
			      just=c("left","bottom"),clip='on'))

    Draw.colourbar(Trange=10)
