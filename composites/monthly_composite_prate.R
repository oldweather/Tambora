# Make summer (JJA) 1816 Monthly precip anomaly composites
...
get.anomaly.composite<-function(date.range,version) {
   ...
   t<-GSDF.reduce.1d(t,'time',mean)
   n<-GSDF.reduce.1d(n,'time',mean)
   c<-t
   c$data[]<-log(n$data/t$data)
   ...
   c$data[]<-pmax(-2,c$data,na.rm=TRUE)
   c$data[]<-pmin(2,c$data,na.rm=TRUE)
...
png(filename="1816_monthly_prate.png",width=412,height=768,pointsize=24)
   p1<-GSDF.plot.2d(c1,x.range=c(150,215),y.range=c(30,70),
                    levels=seq(-2,2,.2),draw=FALSE,
