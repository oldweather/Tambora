# Add data from the 3.6.6 run to the comparison dataset
...
get.reanalysis<-function(index) {
  ...
  v366=get.comparisons(year,month,day,hour,li$lats,li$lons,'3.6.6')
  return(list(v366=v366,indices=li$indices))
}
...
save(Results,file='Results.366.Rdata')

for(i in seq(1:length(Results))) {
   print(i)
   if(is.null(Results[[i]])) next
   Dates<-merge.reanalysis(Dates,i,Results[[i]])
   if(i%%100==0) save(Dates,file='Reanalysis.366.Rdata')
}
save(Dates,file='Reanalysis.366.Rdata')
