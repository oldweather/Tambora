...
# Shows version 3.6.6 (Europe only)
...
Imagedir<-sprintf("%s/images/Tambora.1way+obs",Sys.getenv('SCRATCH'))
...
Draw.obs<-function(year,month,day,hour,Options,Trange=10,size=0.015) {
   ...
   for(station in good.stations) {
      ...
      spread<-Station.anomalies$t2m[[station]]$v366.spread[w[1]]
      t.diff<-t2m-Station.anomalies$t2m[[station]]$v366.mean[w[1]]
...
plot.hour<-function(year,month,day,hour) {    
	...
	prmsl<-TWCR.get.slice.at.hour('prmsl',year,month,day,hour,version='3.6.6')
	prmsl.spread<-TWCR.get.slice.at.hour('prmsl',year,month,day,hour,version='3.6.6',
						  type='spread')
	...
	t2m<-get.temperature.daily(year,month,day,hour,version='3.6.6')
	...
	t2m$data[]<-t2m$data-t2m.normal$data
	   ...
	   pushViewport(dataViewport(c(Options$lon.min,Options$lon.max),
	    ...
	    if(opt$obs=='T') Draw.obs(year,month,day,hour,Options,Trange=10,size=0.025)
