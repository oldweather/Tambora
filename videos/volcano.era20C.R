...
# Pressure fields from ERA20C for 1991/2
...
Imagedir<-sprintf("%s/images/Tambora.ERA20C",Sys.getenv('GSCRATCH'))
...
plot.hour<-function(year,month,day,hour) {    
    ...
    prmsl.normal<-ERA20C.get.slice.at.hour('prmsl',year,month,day,hour,
                                             type='normal')
    t2m<-ERA20C.get.slice.at.hour('air.2m',year,month,day,hour)
    t2m.normal<-ERA20C.get.slice.at.hour('air.2m',year,month,day,hour,
                                             type='normal')
