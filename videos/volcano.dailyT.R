...
if ( is.null(opt$version ) ) { opt$version  = '3.6.6' }
...
Imagedir<-sprintf("%s/images/Tambora.%s",Sys.getenv('SCRATCH'),opt$version)
...
plot.hour<-function(year,month,day,hour) {    
    ...
    t2m<-get.temperature.daily(year,month,day,hour,version=version)
